...
if ($_GET['verwijder'] == 1) {
	...
	$settings = $array_fotos_types[$type];
	...
	// delete crop files
	$sql = "
	SELECT `naam`, `dir`, SUBSTRING(`check_type`,14,100) AS `crop_key` 
	FROM `files` WHERE `check_id` = '".$_GET['id']."' 
	AND `check_type` LIKE 'paginas_foto_%'
	";
	$result = $db->select($sql);
	$rows = $db->row_count;
	if ($rows >= 1) {
	...
	SELECT `naam`, `dir`
	FROM `files` WHERE `check_id` = '".$_GET['id']."' 
	AND `check_type` = 'paginas_galerij'
	";
	$result = $db->select($sql);
	$rows = $db->row_count;
	if ($rows >= 1) {
	...
	AND (
		`check_type` LIKE 'paginas_foto_%'
		OR `check_type` = 'paginas_galerij'
		OR `check_type` = 'paginas'
	)
	";
