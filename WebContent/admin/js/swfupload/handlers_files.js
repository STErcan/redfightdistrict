...
function uploadStart(file) {
	try {
		...
		this.customSettings.filesStatus.show().html('Bestand uploaden...');
	}
	catch (ex) {
		this.debug(ex);
	}
...
function uploadSuccess(file, serverData) {
	try {
		...
		if(status_array.upload_verplaatst) {
			$("#thumbnails").prepend(status_array.file_preview);
			setTimeout(function() {
				var file = $("#x_'"+status_array.file_id+"']");
				$(file).animate({
					opacity:1
				},1000);							
			}, 1000);
